            ...
            try:
                ...
                if self.kullanici_yetki == "kurum_muduru" or not self.kullanici_terapist:
                    df = pd.read_sql_query(
                        """
                        SELECT st.id AS seans_id, st.tarih, COALESCE(st.saat, r.saat, '') AS saat,
                               COALESCE(st.danisan_adi, r.danisan_adi, '') AS danisan_adi,
                               COALESCE(st.terapist, r.terapist, '') AS terapist,
                               COALESCE(r.hizmet_bedeli, st.ucret, 0) AS seans_ucreti,
                               COALESCE(r.alinan_ucret, CASE WHEN COALESCE(st.ucret_alindi,0)=1 THEN COALESCE(st.ucret,0) ELSE 0 END, 0) AS alinan_odeme,
                               COALESCE(r.kalan_borc, CASE WHEN COALESCE(st.ucret_alindi,0)=1 THEN 0 ELSE COALESCE(st.ucret,0) END, 0) AS kalan_borc,
                    ...
                    df = pd.read_sql_query(
                        ...
                        FROM seans_takvimi st
                        LEFT JOIN records r ON r.id = st.record_id OR r.seans_id = st.id
                        WHERE st.tarih>=? AND st.tarih<=? AND COALESCE(st.terapist, r.terapist, '')=?
                        ORDER BY st.tarih, st.saat, st.id
                        """,
