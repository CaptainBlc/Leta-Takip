...
class App(ttk.Window):
    ...
    def _rapor_pencere(self, bas: str, bit: str, title: str):
        ...
        o = ttk.Labelframe(win, text="Özet", padding=10, bootstyle="secondary")
        o.pack(fill=X, padx=10, pady=(0, 10))
        ttk.Label(o, text=f"Seans: {s['seans_sayisi']}").pack(side=LEFT, padx=10)
        ttk.Label(o, text=f"Toplam Bedel: {format_money(s['bedel_toplam'])}").pack(side=LEFT, padx=10)
        ttk.Label(o, text=f"Toplam Ödenen: {format_money(s['alinan_toplam'])}").pack(side=LEFT, padx=10)
        ttk.Label(o, text=f"Toplam Kalan: {format_money(s['kalan_toplam'])}").pack(side=LEFT, padx=10)
        ttk.Label(o, text=f"Kasa Giren: {format_money(s['kasa_giren'])}").pack(side=LEFT, padx=10)
        ttk.Label(o, text=f"Kasa Çıkan: {format_money(s['kasa_cikan'])}").pack(side=LEFT, padx=10)
        ttk.Label(o, text=f"Net: {format_money(net)}", font=("Segoe UI", 10, "bold")).pack(side=LEFT, padx=10)

        # liste (records)
        frame = ttk.Frame(win, padding=10)
        frame.pack(fill=BOTH, expand=True)
        cols = ("Tarih", "Danışan", "Terapist", "Bedel", "Ödenen", "Kalan", "Not")
        tree = ttk.Treeview(frame, columns=cols, show="headings", style="Strong.Treeview")
        for c in cols:
            tree.heading(c, text=c)
            tree.column(c, width=160)
        tree.column("Not", width=360)
            ...
            try:
                ...
                if self.kullanici_yetki == "kurum_muduru" or not self.kullanici_terapist:
                    df = pd.read_sql_query(
                        "SELECT tarih, danisan_adi, terapist, hizmet_bedeli, alinan_ucret, kalan_borc, COALESCE(notlar,'') AS notlar FROM records WHERE tarih>=? AND tarih<=? ORDER BY tarih, id",
                        conn,
                        params=(bas, bit),
                    )
                else:
                    df = pd.read_sql_query(
                        "SELECT tarih, danisan_adi, terapist, hizmet_bedeli, alinan_ucret, kalan_borc, COALESCE(notlar,'') AS notlar FROM records WHERE tarih>=? AND tarih<=? AND terapist=? ORDER BY tarih, id",
                        conn,
                        params=(bas, bit, self.kullanici_terapist),
                    )
                conn.close()
            except Exception:
                df = pd.DataFrame(columns=["tarih", "danisan_adi", "terapist", "hizmet_bedeli", "alinan_ucret", "kalan_borc", "notlar"])
            ...
            for idx, r in df.iterrows():
                ...
                tree.insert(
                    ...
                    values=(
                        ...
                        r["danisan_adi"],
                        r["terapist"],
                        format_money(r["hizmet_bedeli"]),
                        format_money(r["alinan_ucret"]),
                        format_money(r["kalan_borc"]),
                        r["notlar"],
                    ),
                    tags=(tag,),
                )
    ...
    def _build_modules_tab(self):
        # Klasik ana ekran: buradan SEANS TAKİP / HIZLI İŞLEMLER / AYARLAR'a geçilir.
        ...
        ttk.Label(wrapper, text="Lütfen yapmak istediğin işlemi seç:", foreground="gray").pack(anchor=W, pady=(0, 10))

        grid = ttk.Frame(wrapper)
        grid.pack(fill=X)

        def card(parent, title: str, desc: str, cmd, style="primary"):
            c = ttk.Labelframe(parent, text=title, padding=14, bootstyle=style)
            c.pack(side=LEFT, fill=BOTH, expand=True, padx=6)
            ttk.Label(c, text=desc, foreground="gray", wraplength=320).pack(anchor=W, pady=(0, 10))
            ttk.Button(c, text=title, bootstyle=style, command=cmd).pack(fill=X)

        card(
            grid,
            "Seans Takip",
            "Seans kaydı ekle, ödemeyi güncelle, borçları gör.",
            lambda: self.nb.select(self.tab_records),
            "success",
        )

        def _open_quick():
            self._quick_actions_pencere()

        card(
            grid,
            "Hızlı İşlemler",
            "Haftalık rapor, kasa, kılavuz, logo gibi hızlı aksiyonlar.",
            _open_quick,
            "primary",
        )

        if self.kullanici_yetki == "kurum_muduru" and hasattr(self, "tab_settings"):
            card(
                grid,
                "Ayarlar",
                "Terapistler, roller ve kullanıcı yönetimi ayarları.",
                lambda: self.nb.select(self.tab_settings),
                "warning",
            )
    ...
    def _build_records_tab(self):
        ...
        self.ent_danisan = ttk.Entry(top, width=24)
        self.ent_danisan.grid(row=0, column=5, padx=6, pady=6, sticky=W)
        ...
        ttk.Button(top, text="KAYDET", bootstyle="success", command=self.kayit_ekle, width=18).grid(
            row=0, column=8, rowspan=2, padx=10, pady=6, sticky="nsew"
        )
        ...
        self.tree.column("Danışan", width=220)
    ...
    def hizli_seans_kaydi_ekle(self):
        ...
        ent_dan = ttk.Entry(frm, width=34)
        ent_dan.grid(row=1, column=1, sticky=W, padx=6, pady=6)
        ent_dan.focus_set()
        ...
        ttk.Label(frm, text="Hizmet Bedeli (₺):").grid(row=3, column=0, sticky=W, padx=6, pady=6)
        ...
        ttk.Label(frm, text="Alınan (₺):").grid(row=4, column=0, sticky=W, padx=6, pady=6)
        ...
        ttk.Label(frm, text="Not (opsiyonel):").grid(row=5, column=0, sticky=W, padx=6, pady=6)
        ent_not = ttk.Entry(frm, width=34)
        ent_not.grid(row=5, column=1, sticky=W, padx=6, pady=6)

        def _save():
            danisan = (ent_dan.get() or "").strip().upper()
            ...
            try:
                ...
                cur.execute(
                    ...
                    """,
                    (
                        self._tarih_db_from(tarih_var.get()),
                        saat,
                        danisan,
                        terapist,
                        bedel,
                ...
                # takvim ile eşle
                try:
                    self._sync_from_record_to_seans(cur, rid, self._tarih_db_from(tarih_var.get()), saat, danisan, terapist, notlar)
                except Exception:
                    pass
                conn.commit()
                conn.close()
        ...
        btns = ttk.Frame(frm)
        btns.grid(row=6, column=0, columnspan=2, sticky=EW, padx=6, pady=(14, 0))
        ...
        ent_dan.bind("<Return>", lambda e: ent_bedel.focus_set())
    ...
    def kayit_ekle(self):
        danisan = (self.ent_danisan.get() or "").strip().upper()
        ...
        try:
            ...
            # takvim ile eşle
            try:
                self._sync_from_record_to_seans(cur, rid, self._tarih_db(), saat, danisan, terapist, notlar)
            except Exception:
                pass
            conn.commit()
            conn.close()
    ...
    def kayitlari_listele(self):
        ...
        try:
            ...
            if q:
                where.append("danisan_adi LIKE ?")
                params.append(f"%{q}%")

            sql = "SELECT id,tarih,COALESCE(saat,''),danisan_adi,terapist,hizmet_bedeli,alinan_ucret,kalan_borc,notlar FROM records"
            if where:
        ...
        for r in rows:
            ...
            self.tree.insert(
                "",
                END,
                values=(r[0], tarih_g, r[2], r[3], r[4], format_money(r[5]), format_money(r[6]), format_money(r[7]), r[8] or ""),
                tags=(tag,),
            )
    ...
    def odeme_ekle(self):
        ...
        def _save():
            ...
            try:
                conn = self.veritabani_baglan()
                cur = conn.cursor()
                cur.execute("SELECT tarih, danisan_adi, terapist, hizmet_bedeli, alinan_ucret FROM records WHERE id=?", (rid,))
                row = cur.fetchone()
                if not row:
                    conn.close()
                    win.destroy()
                    return
                kayit_tarih = (row[0] or "").strip()
                danisan = (row[1] or "").strip()
                terapist = (row[2] or "").strip()
                bedel = float(row[3] or 0)
                alinan_eski = float(row[4] or 0)
                alinan_yeni = alinan_eski + ek
                kalan_yeni = max(0.0, bedel - alinan_yeni)
                cur.execute(
                    "UPDATE records SET alinan_ucret=?, kalan_borc=?, olusturma_tarihi=? WHERE id=?",
                    (alinan_yeni, kalan_yeni, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), rid),
                )

                # Tahsilat geçmişi + kasa defteri kaydı
                ...
                try:
                    cur.execute(
                        """
                        INSERT INTO odeme_hareketleri (record_id, tarih, tutar, odeme_sekli, aciklama, olusturan_kullanici_id, olusturma_tarihi)
                        VALUES (?,?,?,?,?,?,?)
                        """,
                        (
                            rid,
    ...
    def kayit_sil(self):
        ...
        try:
            conn = self.veritabani_baglan()
            cur = conn.cursor()
            # varsa bağlı seansı da sil
            try:
                cur.execute("SELECT COALESCE(seans_id, NULL) FROM records WHERE id=?", (rid,))
                sid = (cur.fetchone() or [None])[0]
                if sid:
                    cur.execute("DELETE FROM seans_takvimi WHERE id=?", (sid,))
                else:
                    # record_id ile bağlanmış olabilir
                    cur.execute("DELETE FROM seans_takvimi WHERE record_id=?", (rid,))
            except Exception:
                pass
